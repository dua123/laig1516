...
MyCilinderLateral.prototype.initBuffers = function() {
	...
	for (var j = 0; j <= this.sec_height; j++) {
		for (var i = 0; i <= this.parts_sec; i++) {
			...
			var radius = this.bot_rad + this.radius_diff * j;
			var ang = i * angle;
			/*var x1=Math.cos(ang) * radius;
			var y1=Math.sin(ang) * radius;
			var z1=(this.height * j) / this.sec_height;*/
			this.vertices.push(Math.cos(ang) * radius, Math.sin(ang) * radius, (this.height * j) / this.sec_height);
			this.normals.push(Math.cos(ang) * radius, Math.sin(ang) * radius, 0);
			this.texCoords.push(i * patchS, j * patchT);
			/*if(j==this.sec_height){
...
MyCylinderLateral.prototype.updateTexCoords = function() {
	this.texCoords = [];

	for (var st = 0; st <= this.sec_height; st++) {
		var n = (this.parts_sec + 1) * st;
		var radius = this.bot_rad + this.radius_diff * st;
		for (var sl = 0; sl<= this.parts_sec; sl++) {
			this.texCoords.push(this.angle * sl, (st * this.height)/sec_height);
		}
	}
